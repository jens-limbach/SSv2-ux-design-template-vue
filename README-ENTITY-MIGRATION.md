...
Access the OpenAPI schema using a two-step process:
...
This returns a list of all available OpenAPI schemas in the format:
```
{serviceName}.openapischema.{entityName}
```

**Step 2: Fetch specific entity schema**
```
...
- **Best Practice**: Use the list endpoint first to find all matching schemas, then select the correct one
- If multiple matches exist, ask the user to provide the exact schema name

**Example for Opportunities:**

1. List schemas and search for "opportunity":
   ```
   GET /sap/c4c/api/v1/repository-service/openApiSchemas/
   ```

2. Common results might include:
   - `sap.crm.opportunityservice.openapischema.opportunities`
   - `sap.crm.opportunityengagementservice.openapischema.opportunities`

3. Fetch the correct one:
   ```
   GET /sap/c4c/api/v1/repository-service/openApiSchemas/sap.crm.opportunityengagementservice.openapischema.opportunities
   ```

**Workflow When Helping Users:**
1. Ask user for entity name (e.g., "opportunities")
...
5. If no matches or unclear: ask user to provide exact schema name
